...
[parameter table](#parameters) further down — a 16 mm conduit with 18 mm wall hole, 200°
coverage, and a corrugation profile measured off a real pipe (pitch 3.79 mm: 1.2 mm valley
+ 2.59 mm crest). Only the camera angle differs between the 3D views.
...
| `corr_chamfer` | `0.3` | Bevel on the tooth/recess edges (mm); `0` = sharp square teeth |
...
- Real conduit crests and valleys are rounded/U-shaped rather than square. `corr_chamfer`
  bevels the tooth and recess edges to match (set it to `0` for sharp square teeth). It must
  be ≤ half the smaller of `corr_width`/`groove_width` (asserted in the model).
...
- Length = `corr_count · (corr_width + groove_width)` (always whole corrugation periods).
  The defaults were measured off a real 16 mm conduit: 6 corrugations spanned 22.74 mm
  (pitch 3.79 mm = 1.2 mm valley + 2.59 mm crest).
