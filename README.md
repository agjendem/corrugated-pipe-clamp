...
| `corr_round` | `0.1` | Fillet radius on the tooth/recess edges (mm); `0` = sharp corners |
...
- Real conduit crests and valleys are rounded/U-shaped rather than square. `corr_round`
  fillets the tooth and recess edges with a true radius to match (set it to `0` for sharp
  corners). It must be ≤ half the smaller of `corr_width`/`groove_width` and ≤ `corr_depth`
  (both asserted in the model).
